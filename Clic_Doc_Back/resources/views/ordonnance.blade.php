...
<body onload="window.print()">
    ...
    <div class="m-6 content" style="font-family: 'Kalam', cursive;">
        ...
        <div class="container-prescription">
            @if($item->medicament)
                <p class="font-bold">{{ $counter }}. {{ $item->medicament }}</p>
                @php
            ...
            <p class="ml-5">
                ...
                @if($item->matin || $item->midi || $item->soir || $item->au_coucher)
                    (
                    @if($item->matin) {{ $item->matin }} le matin @endif
                    @if($item->matin && $item->midi) - @endif
                    @if($item->midi) {{ $item->midi }} à midi @endif
                    @if(($item->matin || $item->midi) && $item->soir) - @endif
                    @if($item->soir) {{ $item->soir }} le soir @endif
                    @if(($item->matin || $item->midi || $item->soir) && $item->au_coucher) - @endif
                    @if($item->au_coucher) {{ $item->au_coucher }} au coucher @endif
                    )
                @endif
                ...
                @if($item->duration_value && $item->duration_unit)
                    pendant {{ $item->duration_value }} {{ $item->duration_unit }}
                @endif
            </p>
            
            @if($item->commentaire)
                <p class="ml-5 italic">{{ $item->commentaire }}</p>
            @endif
